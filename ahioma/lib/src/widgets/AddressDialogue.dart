...
class AddressDialog extends StatefulWidget {
  ...
  AddressDialog({Key key, this.user, this.onChanged,this.uId}) : super(key: key);

  @override
  _AddressDialogState createState() =>
      _AddressDialogState(this.user);
}

class _AddressDialogState extends State<AddressDialog> {
  _AddressDialogState(User _user) {
    this._user = _user;
  }
  User _user;
  ...
  void _submit() async {
    if (_AddressFormKey.currentState.validate()) {
      ...
      update = 'Adding new address...';
      loadig.value+=1;
      print(widget.uId);
      NewAddress updat = NewAddress(
          address: street.text, state: state.text, localGovernment: lga.text);
      await updat.addAddress(widget.uId);
      if (updat.status == 'true') {
        update = 'SUCCESSFUL';
        loadig.value+=1;
        Navigator.pop(context);
      } else {
